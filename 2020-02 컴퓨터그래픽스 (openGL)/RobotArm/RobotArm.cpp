...
int RobotControl = 0;
int TeapotToggle = 0;      // SPACE�� �̿��� Ƽ�� ��ǿ� ���Ǵ� ���� ( 0:��Ȱ��ȭ  1: Ȱ��ȭ  -1: �ӽû���(�Ʒ��� ���� ����))
...
const char* ourObjectPath = "./orange.obj";
// translate it so it's at the center of the scene
// it's a bit too big for our scene, so scale it down
glm::mat4 objectXform = glm::scale(glm::rotate(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)), -90.0f, glm::vec3(0.0f, 1.0f, 0.0f)), glm::vec3(0.03f, 0.03f, 0.03f));
glm::mat4 teapotTrans;       // space�� �Է��� ������ Ƽ���� ��ġ�� �ӽ÷� �����ϴ� ���
...
void myDisplay()
{
	...
	model = glm::rotate(model, glm::radians(BaseSpin), glm::vec3(0, 1.0f, 0));
	DrawBase(model);

	//Arm1 rendering
	model = glm::translate(model, glm::vec3(0, 0.4f, 0));
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0, 0, 1.0f));
	DrawArmSegment(model);

	//Arm2 rendering
	model = glm::translate(model, glm::vec3(0, 0.5f, 0));
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(0, 0, 1.0f));
	DrawArmSegment(model);

	//Wrist rendering
	model = glm::translate(model, glm::vec3(0, 0.5f, 0));
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(0, 0, 1.0f));
	DrawWrist(model);


	model = glm::translate(model, glm::vec3(0, 0.2f, 0));
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0, 1.0f, 0));

	//teapot
	if (TeapotToggle == -1) {							//teapotToggle�� �ӽû����̸� �� ���� ����
		teapotTrans = inverse(model)*objectXform;		//objectXform �� model�� ������� ���ϰ� (�ʱ���� ������ ����)
		TeapotToggle += 2;								//teapotToggle�� Ȱ��ȭ��Ŵ
	}

	if (TeapotToggle == 1) {							//teapotToggle �� Ȱ��ȭ�����϶�
		objectXform = model * teapotTrans;				//model�� transformation�� �Ȱ��� ������
	}
	DrawObject(objectXform);							//teapot�� �׸�


														//Finger rengering
	glm::mat4 modelCopy = model;						//�հ��� �������� �κ��� ��ġ�� ����ϴ� ����

	model = glm::rotate(model, glm::radians(FingerAng1), glm::vec3(0, 0, 1.0f));
	DrawFingerBase(model);

	model = glm::translate(model, glm::vec3(0, 0.35f, 0));
	model = glm::rotate(model, glm::radians(FingerAng2), glm::vec3(0, 0, 1.0f));
	DrawFingerTip(model);

	model = modelCopy;									//�հ��� �������� ��ġ�� ���ƿͼ� �ι�° �հ����� �׸�
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		PhongShader->setVec3("lightDirection", camera.Front);
		PhongShader->setFloat("lightCutOff", glm::cos(glm::radians(12.5f)));
		PhongShader->setFloat("lightOuterCutOff", glm::cos(glm::radians(17.5f)));
		PhongShader->setFloat("constant", 1.0f);
		PhongShader->setFloat("linear", 0.09f);
		PhongShader->setFloat("quadratic", 0.032f);
		PhongShader->setBool("angular", angular);
		...
		FloorShader->setVec3("lightDirection", camera.Front);
		FloorShader->setFloat("lightCutOff", glm::cos(glm::radians(12.5f)));
		FloorShader->setFloat("lightOuterCutOff", glm::cos(glm::radians(17.5f)));
		FloorShader->setFloat("constant", 1.0f);
		FloorShader->setFloat("linear", 0.09f);
		FloorShader->setFloat("quadratic", 0.032f);
		FloorShader->setBool("angular", angular);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (RightButtonDown) {
		camera.ProcessMouseMovement(1000 * xoffset, 1000 * yoffset);
	}
