			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>((x + n)*kernel_size + (y + n), 0) = input.at<C>(tempa, tempb)[0];
						kernel.at<G>((x + n)*kernel_size + (y + n), 1) = input.at<C>(tempa, tempb)[1];
						kernel.at<G>((x + n)*kernel_size + (y + n), 2) = input.at<C>(tempa, tempb)[2];
					}
				}
			}
