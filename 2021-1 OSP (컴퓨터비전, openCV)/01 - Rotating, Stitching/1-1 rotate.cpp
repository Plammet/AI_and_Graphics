		...
		for (int j = 0; j < sq_col; j++) {
			...
			if ((y >= 0) && (y <= (row - 1)) && (x >= 0) && (x <= (col - 1))) {
				if (!strcmp(opt, "nearest")) {
					...
					output.at<T>(i, j) = (Ydivide) * (input.at<T>(ceil(y), ceil(x)) * Xdivide + input.at<T>(ceil(y), floor(x)) * (1 - Xdivide))
						+ (1 - Ydivide) * (input.at<T>(floor(y), ceil(x)) * Xdivide + input.at<T>(floor(y), floor(x)) * (1 - Xdivide));
				}
			}
		}
	}
