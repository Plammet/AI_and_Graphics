...
def conv(in_channels, out_channels):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),  # 3은 kernel size
...
class Unet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x, conv4], dim =1) #######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp4(x)
        x = self.upsample(x)
        x = torch.cat([x, conv3], dim=1)   #######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp3(x)
        x = self.upsample(x)
        x = torch.cat([x, conv2], dim=1)   #######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp2(x)
        x = self.upsample(x)
        x = torch.cat([x, conv1], dim=1)   #######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp1(x)
