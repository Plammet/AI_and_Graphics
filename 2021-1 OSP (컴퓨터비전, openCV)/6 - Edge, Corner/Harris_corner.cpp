...
float magnitude(const Mat input, int i, int j);
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++) {
				if (corner_mat.at<uchar>(i, j) == 1) {
					//input_visual.at<Vec3b>(i, j)[0] = 255;		input_visual.at<Vec3b>(i, j)[1] = 0;	input_visual.at<Vec3b>(i, j)[2] = 255;					
					circle(input_visual, Point(j, i), 2, Scalar(0, 0, 255), 1, 8, 0);	//You can also use this function of drawing a circle. For details, search 'circle' in OpenCV.
					corner_num++;
				}
			}
			...
			if (corner_mat.at<uchar>(i-radius, j-radius) == 1) {
				...
				for (int a = -radius; a <= radius; a++) {
					for (int b = -radius; b <= radius; b++) {
						if (i + a >= radius && i + a < row + radius && j + b >= radius && j + b < col + radius) {
...
float magnitude(const Mat input, int i, int j) {
	...
	for (int a = -1; a <= 1; a++) {
		for (int b = -1; b <= 1; b++) {
			Gx += Sx[a + 1][b + 1] * input.at<float>(i + a, j + b);
			Gy += Sy[a + 1][b + 1] * input.at<float>(i + a, j + b);
		}
	}
